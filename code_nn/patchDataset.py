...
# 4. Spatial Diversity: Patches must be at least 100 pixels apart from each other.
# 5. No Padding: If a patch hits the image border, the crop window slides inwards to stay strictly inside the image.
...
# High worker count because this is Disk I/O and CPU bound
MAX_WORKERS = 32 
...
# 2. Process Test Split directly
crop_size = 256
min_distance = 100 # Minimum pixels between patch centers to ensure diversity

for target_split in splits:
    fs_dir = os.path.join(base_dir, f'{target_split}_masks', 'fastsam')
    gt_dir = os.path.join(base_dir, f'{target_split}_masks', 'gt')
    img_dir = os.path.join(base_dir, target_split)
    
    if not os.path.exists(fs_dir):
        print(f"Directory {fs_dir} not found. Skipping.")
        continue
        
    mask_files = [f for f in os.listdir(fs_dir) if f.endswith('.png')]
    print(f"Extracting diverse, unpadded patches from {target_split} ({len(mask_files)} masks found)...")
    ...
    def process_mask(mask_name):
        ...
        # Skip image entirely if it's smaller than the crop size
        ...
        for (cy, cx) in boundary_points:
            ...
            save_mask_name = f"{base_no_ext}_{patch_suffix}.png"
            save_img_name = f"{base_no_ext}_{patch_suffix}.jpg"
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        results = list(tqdm(executor.map(process_mask, mask_files), total=len(mask_files), desc=f"Extracting {target_split}"))
        
    total_extracted = sum(results)
    print(f" -> Successfully extracted {total_extracted} valid patches for {target_split}.")

print("\nPatch dataset generation complete! Diverse patches with zero black padding.")
